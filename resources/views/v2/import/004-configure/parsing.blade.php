...
@section('scripts')
    <script type="text/javascript">
        window.onload = function () {
            const parseUrl = '{{ route('configure-import.index', [$identifier]) }}?parse=true';
            ...
            const startParsing = async function () {
                ...
                try {
                    ...
                    if (!response.ok || payload.success !== true) {
                        ...
                        window.location.href = payload.redirect || '{{ route('new-import.index', ['file']) }}';
                        return;
                    }
                    setProgress(100);
                    statusLine.textContent = payload.message || 'Parsing finished. Redirecting...';
                    window.location.href = payload.redirect || '{{ route('configure-import.index', [$identifier]) }}';
                } catch (error) {
